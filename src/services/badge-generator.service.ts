...
export class BadgeGeneratorService {
  ...
  private widenMessageSegment(svgString: string, minMessageWidth: number): string {
    ...
    updated = updated.replace(
      new RegExp(`<rect x="${labelWidth}" width="\\d+" height="(\\d+)" fill="([^"]+)"/>`),
      `<rect x="${labelWidth}" width="${newMessageWidth}" height="$1" fill="$2"/>`
    );

    const textMatches = Array.from(
      updated.matchAll(/<text x="(\d+)" y="(\d+)" transform="scale\(\.1\)"[^>]*>/g)
    );
